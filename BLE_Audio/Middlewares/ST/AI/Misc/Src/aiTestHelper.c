...
{
  if (fct) {
	  STBOX1_PRINTF("E: AI error (%s) - type=0x%02x code=0x%02x\r\n", fct,
        err.type, err.code);
  } else {
	  STBOX1_PRINTF("E: AI error - type=0x%02x code=0x%02x\r\n", err.type, err.code);
  }
}
...
static inline void aiPrintDataType(const ai_buffer_format fmt)
{
    if (AI_BUFFER_FMT_GET_TYPE(fmt) == AI_BUFFER_FMT_TYPE_FLOAT) {
    	STBOX1_PRINTF("float%d", (int)AI_BUFFER_FMT_GET_BITS(fmt));
    } else if (AI_BUFFER_FMT_GET_TYPE(fmt) == AI_BUFFER_FMT_TYPE_BOOL) {
    	STBOX1_PRINTF("bool%d", (int)AI_BUFFER_FMT_GET_BITS(fmt));
    } else { /* integer type */
    	STBOX1_PRINTF("%s%d", AI_BUFFER_FMT_GET_SIGN(fmt)?"i":"u",
...
void aiPrintBufferInfo(const ai_buffer *buffer)
{
  ...
  {
	  STBOX1_PRINTF("%d)", (int)AI_BUFFER_SHAPE_ELEM(buffer, AI_SHAPE_CHANNEL));
  }

  STBOX1_PRINTF("%d/", (int)AI_BUFFER_SIZE(buffer));
  ...
  if (AI_BUFFER_FMT_GET_TYPE(fmt) == AI_BUFFER_FMT_TYPE_Q) {
    if (AI_BUFFER_META_INFO_INTQ(buffer->meta_info)) {
      ...
      for (int idx=0; idx<max_; idx++) {
        ...
        STBOX1_PRINTF("(%f,%d),", (double)scale, zero_point);
      }
      STBOX1_PRINTF((s_ > max_)?"..":"");
    } else if (AI_BUFFER_FMT_GET_BITS(fmt) < 8) {
      /* lower of 8b format */
    	STBOX1_PRINTF(" int32-%db", (int)AI_BUFFER_FMT_GET_BITS(fmt));
    } else {
    	STBOX1_PRINTF(" Q%d.%d",
...
{
	STBOX1_PRINTF("Network informations...\r\n");
	STBOX1_PRINTF(" model name         : %s\r\n", report->model_name);
	STBOX1_PRINTF(" model signature    : %s\r\n", report->model_signature);
	STBOX1_PRINTF(" model datetime     : %s\r\n", report->model_datetime);
	STBOX1_PRINTF(" compile datetime   : %s\r\n", report->compile_datetime);
	STBOX1_PRINTF(" runtime version    : %d.%d.%d\r\n",
      report->runtime_version.major,
      report->runtime_version.minor,
      report->runtime_version.micro);
  if (report->tool_revision[0])
	  STBOX1_PRINTF(" Tool revision      : %s\r\n", (report->tool_revision[0])?report->tool_revision:"");
  STBOX1_PRINTF(" tools version      : %d.%d.%d\r\n",
      report->tool_version.major,
      report->tool_version.minor,
      report->tool_version.micro);
  STBOX1_PRINTF(" complexity         : %lu MACC\r\n", (unsigned long)report->n_macc);
  STBOX1_PRINTF(" c-nodes            : %d\r\n", (int)report->n_nodes);

  STBOX1_PRINTF(" map_activations    : %d\r\n", report->map_activations.size);
  ...
  STBOX1_PRINTF(" n_inputs/n_outputs : %u/%u\r\n", report->n_inputs,
          report->n_outputs);
